...
def test_end_to_end_troubleshooting_flow() -> None:
    """Verify the complete agent-to-coordinator troubleshooting flow."""
    ...
    coordinator = TroubleshootingCoordinator(
        agents=[
            CPUAgent(llm_client=cpu_llm),
            MemoryAgent(llm_client=memory_llm),
            NetworkAgent(llm_client=network_llm),
        ]
    )
